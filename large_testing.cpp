...
const int dim = 1024;
const int max_elements = 960000;
const int num_centroids = 1600;
const std::vector<int> thread_tests = {60, 80, 100, 120, 160, 200, 224};
...
private:
    ...
    int calculate_optimal_threads(size_t data_size) {
        ...
        if (data_size > 500000) {
            return std::min(200, max_threads);
...
public:
    std::vector<std::vector<float>> compute_inner_products_threaded(
        ...
        for (int t = 0; t < num_threads; ++t) {
            ...
            std::cout << "Thread " << t << " processing vectors " << start_idx << " to " << end_idx - 1 << std::endl;
        ...
        // Print thread timing info
        std::cout << "Thread timing breakdown:" << std::endl;
        for (int t = 0; t < num_threads; ++t) {
            if (thread_results[t].success) {
                std::cout << "  Thread " << t << ": " 
                          << std::chrono::duration_cast<std::chrono::milliseconds>(thread_results[t].compute_time).count() 
                          << " ms" << std::endl;
            }
        }
...
int main()
{
    ...
    // Print HNSWLIB thread info
    calculator.printThreadInfo();
    
    // Test HNSWLIB with different thread counts
    std::vector<int> hnswlib_thread_counts = thread_tests;
    ...
    // Find optimal HNSWLIB thread count
//    std::cout << "\nFinding optimal HNSWLIB configuration:" << std::endl;
//    int optimal_hnswlib_threads = calculator.findOptimalThreadCount(random_centroids, data);
    
    // 4. Scalar comparison (for smaller subset to avoid long runtime)
    std::cout << "\n4. Scalar Implementation (subset for speed):" << std::endl;
    std::vector<std::vector<float>> data_subset(data.begin(), data.begin() + std::min(10000, (int)data.size()));
    
    auto scalar_start = std::chrono::high_resolution_clock::now();
    ScalarInnerProduct scalar_calculator;
    std::vector<std::vector<float>> scalar_results = scalar_calculator.compute(random_centroids, data_subset);
    ...
    std::cout << "Scalar (subset) took: " << scalar_duration.count() << " microseconds" << std::endl;
    ...
    // Summary
    std::cout << "\n=== SUMMARY ===" << std::endl;
    std::cout << "Dataset: " << data.size() << " vectors x " << dim << " dimensions" << std::endl;
    std::cout << "Centroids: " << random_centroids.size() << std::endl;
    std::cout << "Hardware threads: " << std::thread::hardware_concurrency() << std::endl;
    std::cout << "Single AMX: " << single_duration.count() << " μs" << std::endl;
    std::cout << "Multi AMX: " << threaded_duration.count() << " μs" << std::endl;
    if (!hnswlib_times.empty()) {
        std::cout << "Best HNSWLIB: " << *std::min_element(hnswlib_times.begin(), hnswlib_times.end()) << " μs" << std::endl;
    }
